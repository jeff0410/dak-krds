...
const tooltipInstances = new Map<string, { setOpen: (open: boolean) => void }>();
...
const CustomTooltip = styled(
  ({
    ...
    zIndex,
    ...props
  }: TooltipProps & { maxWidth?: number; backgroundColor?: string; zIndex?: number }) => (
...
export const ClickTooltip = ({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (open) {
        ...
        if (!target.closest('[role="tooltip"]') && !target.closest('[data-tooltip-trigger]')) {
          handleClose();
        }
      }
    };
  ...
  const tooltipContent = (
    <div
      ...
      role='dialog'
      aria-label='오류 설명'
      style={{ position: 'relative', minHeight: '20px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
        <button ref={closeButtonRef} onClick={handleClose} aria-label='툴팁 닫기'>
          <Icon icon='Close' />
        </button>
      </div>
