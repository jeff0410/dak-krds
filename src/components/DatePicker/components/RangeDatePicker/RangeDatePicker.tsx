...
export function RangeDatePicker({
	...
	useEffect(() => {
		...
		const positioning = () => {
			if (calendarRef.current && endInputRef.current && usePortal) {
				...
				const calendarWidth =
					calendarRef.current.firstElementChild?.getBoundingClientRect()
						?.width ?? 328;
				if (calendarWidth <= 0) return;
				const diff = Math.abs(calendarWidth - inputRect.width);
				const left =
					inputRect.left + (calendarWidth > inputRect.width ? -1 : 1) * diff;
	...
	}, [usePortal, calPosition, portalGap, scrollableElementId]);
		...
		>
			...
			<div className={styles.rangeInputContainer}>
				...
				<TextInput
					...
					gap={0}
				/>
				{usePortal && rootElement
					? createPortal(calendarContent, rootElement)
					: calendarContent}
			</div>
		</div>
	);
