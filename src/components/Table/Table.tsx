...
export function Table<T extends MRT_RowData>({
			...
			if (
				...
				return {
					...
					Cell: ({
						...
						return (
							<div
								className={styles.toggleColumnStyle}
								key={(row as { id: string }).id}
							>
								<div className={styles.toggleColumnLabel}>
									{cellContent as React.ReactNode}
								</div>
								<div className={styles.toggleColumnIcon}>
									<Icon
										icon={
											(row as { getIsExpanded: () => boolean }).getIsExpanded()
												? "ArrowUp"
												: "ArrowDown"
										}
										size={20}
	...
	const table = useMaterialReactTable({
		...
		renderEmptyRowsFallback: () => (
			<div className={styles.emptyTable}>{noData}</div>
		),
		...
		renderDetailPanel: accordionContent
			? ({ row }) => accordionContent(row)
			: undefined,
