...
| ✅ | 파비콘 | Favicon | `Favicon` — head 에 link/meta 주입, 라이트·다크 분기, apple-touch-icon · manifest · theme-color |
...
KRDS 55개 컴포넌트를 모두 제공합니다. 앞으로는 신규 컴포넌트 추가보다
아래 항목을 다룹니다.

- KRDS 문서 개정 시 기존 컴포넌트 명세 반영
- 접근성 검증 (명도 대비 · 키보드 탐색 · 스크린 리더)
- 컴포넌트별 사용 예제 문서 보강
